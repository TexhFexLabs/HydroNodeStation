...
static bool migrated, power_good=true;
static int budget=-1;
...
bool NvmPort_MarkMigrated(void) {if(!step())return false;migrated=true;return true;}
...
int main(void)
{
    ...
    power_good=false;assert(!Nvm_Write(0,&new,4));power_good=true;
    assert(!Nvm_Write(NVM_PAYLOAD_BYTES-1,&new,4));
    assert(!Nvm_Read(UINT32_MAX,&read,4));
    puts("NVM: 208 power-cut cases, CRC fallback, no stale legacy rollback, voltage and bounds passed");
}
