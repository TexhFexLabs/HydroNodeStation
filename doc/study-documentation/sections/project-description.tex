...
\begin{itemize}
  ...
  \item \textbf{Partikelmaterie:} Sensirion SPS30 (PM1.0, PM2.5, PM4, PM10 und Zahlkonzentration)
  \item \textbf{Temperatur und relative Feuchte:} Sensirion SHT45 ($\pm 0{,}1$ °C / $\pm 1{,}0$ \% rH)
  \item \textbf{Luftdruck:} Bosch BMP390 ($\pm 0{,}5$ hPa absolut)
  \item \textbf{UV-Index:} Lite-On LTR390 (UVA und Umgebungslicht)
  \item \textbf{Batterieladezustand:} Maxim MAX17048 (Fuel Gauge über I2C)
\end{itemize}

Alle Sensoren werden über einen gemeinsamen I2C-Bus verwaltet. Zur Optimierung des Stromverbrauchs zwischen Übertragungen versetzt die Firmware die Sensoren mit Schlafmodus (SPS30 und SCD41) über entsprechende I2C-Befehle in den stromsparenden Zustand, während Sensoren ohne Schlafkommando (etwa der SHT45) zwischen den Einzelmessungen von selbst auf niedrigem Stromniveau ruhen. Anschließend versetzt sich der Mikrocontroller in den Tiefschlaf (STOP2), sodass die Ruhestromaufnahme des Gesamtsystems unter 20 µA liegt.
...
Das Herzstück der Station bildet der \textbf{STM32WLE5CCU6}, ein SoC (System-on-Chip), der einen ARM Cortex-M4-Anwendungskern mit einem Sub-GHz-LoRa-Funktransceiver integriert. Im Gegensatz zu modularen Lösungen ermöglicht die Verwendung des Bare-Die-Chips eine vollständige Kontrolle über das RF-Frontend-Layout, minimiert den Platzbedarf und vermeidet die Kostenaufschläge fertiger Module. Ein eigener RF-Pfad mit BALFHB-WL-02D3-Balun und BGS12SN6-Funkschalter sorgt für korrekt angepasste TX- und RX-Pfade bei 868 MHz (EU-Frequenzband).
...
Die Elektronik ist in einem versiegelten Gehäuse nach Schutzart IP65 untergebracht. Ein 3D-gedrucktes Stevenson-Schutzgitter aus weißem, UV-beständigem ASA-Kunststoff sorgt für eine freie Luftzirkulation an den Sensoren und schützt sie zugleich vor direkter Sonneneinstrahlung und Regen. Eine wasserdichte Kabeleinführung nimmt die Solarmodulzuleitung auf. Die Station ist für die Befestigung an einer üblichen Stange oder einer Balkonbrüstung konzipiert und kann monatelang unbeaufsichtigt betrieben werden.
